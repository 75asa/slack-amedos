...
app.command(`/${slashCommandName}`, async ({ command, ack, context }) => {
  ...
  if (mode === "now") {
    ...
    const loadingMessageTs: string | undefined = await callChatPostMessage(
      chatPostMessageArgs
    )
      .then(extractMessageTs)
      .catch(error => {
        printCompleteJSON(error);
        chatPostMessageArgs.asUser = false;
        return callChatPostMessage(chatPostMessageArgs)
          .then(extractMessageTs)
          .catch(error => {
            printCompleteJSON(error);
            return undefined;
    ...
    Promise.all(imageFetchRequests)
      .then(images =>
        Promise.all(
...
function buildImageUrl(args: ImageUrlArgs): string {
  return `https://map.yahooapis.jp/map/V1/static?appid=${yahooAppId}&z=10&lat=${
    args.lat
  }&lon=${args.lon}&width=${args.width}&height=${
    args.height
  }&mode=${yahooMapMode}&overlay=type:rainfall|datelabel:on|date:${args.m.format(
    "YYYYMMDDHHmm"
  )}`;
}
...
function uploadImage(args: UploadImageArgs): Promise<void> {
  return app.client.files
    .upload({
      ...
      file: args.file,
      filename: `ameds_${args.prefName}.${args.filetype}`,
